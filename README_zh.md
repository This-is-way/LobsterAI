...
<p align="center">
  <strong>7×24 小时帮你干活的全场景个人助理，由 Johnson-WEB&APP 开发</strong>
</p>
...
**Johnson-Claw** 是由 Johnson-WEB&APP 开发的全场景个人助理 Agent。它 7×24 小时待命，能够帮你完成日常办公中的各类事务 —— 数据分析、制作 PPT、生成视频、撰写文档、搜索信息、收发邮件、定时任务，以及更多。

Johnson-Claw 的核心是 **Cowork 模式**，它能在本地或沙箱环境中执行工具、操作文件、运行命令，一切都在你的监督下自主完成。此外，Johnson-Claw 支持通过钉钉、飞书、Telegram、Discord 等 IM 平台远程触发，让你在手机上也能随时指挥 Agent 工作。
...
该运行时以解释器为主，不预装 Johnson-Claw 技能所需的 Python 三方包；相关依赖可在运行时按需安装。
...
Johnson-Claw 采用 Electron 严格进程隔离架构，所有跨进程通信通过 IPC 完成。
...
Cowork 是 Johnson-Claw 的核心功能 —— 基于 Claude Agent SDK 的 AI 工作会话系统。它面向办公场景设计，能够自主完成数据分析、文档生成、信息检索等复杂任务。
...
Johnson-Claw 内置 16 种技能，覆盖办公、创作、自动化等多种场景，通过 `SKILLs/skills.config.json` 配置启停和排序：
...
Johnson-Claw 支持创建定时任务，让 Agent 按计划自动执行重复性工作。
...
Johnson-Claw 支持将 Agent 桥接到多种 IM 平台。在手机上通过 IM 发送消息即可远程触发桌面端的 Agent 执行任务，随时随地指挥你的个人助理。
...
Johnson-Claw 内置记忆系统，能够跨会话记住你的个人信息和偏好，让 Agent 越用越懂你。

### 记忆获取方式

- **自动提取** — 对话过程中，系统自动识别并记录你的个人信息（姓名、职业等）、偏好习惯（喜好的语言、格式、风格）和个人事实（养的宠物、使用的工具等），无需手动操作
- **主动告知** — 在对话中直接说「记住我喜欢用 Markdown 格式」「记下我的项目叫 Johnson-Claw」等，Agent 会以更高置信度存储
